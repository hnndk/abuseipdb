...
class IrisAbuseIPDBInterface(IrisModuleInterface):
    ...
    def register_hooks(self, module_id: int):
        ...
        try:
            if module_conf.get('abuseipdb_on_create_hook_enabled', False):
            ...
            if module_conf.get('abuseipdb_on_update_hook_enabled', False):
            ...
            if module_conf.get('abuseipdb_manual_hook_enabled', True):
            ...
            self.log.error(f"Error in register_hooks: {str(e)}")
    ...
    def hooks_handler(self, hook_name: str, hook_ui_name: str, data: any):
        ...
        try:
            ...
            self.log.error(f"Exception in hooks_handler: {str(e)}")
    ...
    def _handle_ioc(self, data) -> InterfaceStatus.IIStatus:
        ...
        """
        try:
            abuse_handler = AbuseIPDBHandler(
            ...
            for element in data:
                try:
                    ...
                    ioc_type = getattr(element, 'ioc_type', None)
                    if ioc_type:
                        type_name = getattr(ioc_type, 'type_name', '')
                        if 'ip-' in type_name:
                            status = abuse_handler.handle_abuseipdb_ip(ioc=element)
                            in_status = InterfaceStatus.merge_status(in_status, status)
                        else:
                            self.log.info(f'IOC type {type_name} not handled by AbuseIPDB module. Skipping')
                    else:
                        self.log.warning('IOC type not found in element')
                except Exception as e:
                    self.log.error(f"Error processing IOC: {str(e)}")
            ...
            self.log.error(f"Error in _handle_ioc: {str(e)}")
            self.log.error(traceback.format_exc())
            return InterfaceStatus.I2Error(f"Error in _handle_ioc: {str(e)}")